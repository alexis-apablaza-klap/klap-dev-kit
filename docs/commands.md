...
|---|---|---|
| `/klap:trabajar-hu <ISSUE-KEY>` | Flujo completo de 8 fases: contexto → análisis → diseño → implementación → validación → certificación → documentación → finalización. | Es el flujo por defecto para cualquier HU real. |
...
| `/klap:documentar [ISSUE-KEY]` | Actualiza memoria del repo y, si corresponde, Confluence + targeted-sync. | Cerrar la documentación de un cambio que ya se implementó fuera del flujo completo. |
| `/klap:actualizar-componente [repo]` | Revisa el repo y propone/actualiza `component.yaml` y `docs/context/index.yaml`. | Onboarding de un repo nuevo al Dev-Kit, o poner al día la memoria de un repo que quedó desactualizada. |
